...
def do_let_form(vals, env):
    ...
    for binding in bindings:
        #print(binding[0])
        #print(scheme_symbolp(binding[0]))
        #print(type(binding))
        if len(binding) != 2 or not scheme_symbolp(binding[0]):
           raise SchemeError
        name = binding[0]
...
def do_if_form(vals, env):
    ...
    decision = vals.first
    t = vals.second.first
    f = vals.second.second.first
    #print("decision: ", decision)
    if scheme_true(scheme_eval(decision, env)):
        return t
    else:
        return f
...
def check_formals(formals):
    ...
    if not scheme_listp(formals):
        ...
        raise SchemeError
    while len(formals) != 0:
        #print("formals.second", formals.second)
        if formals.first in formal or not scheme_symbolp(formals.first):
            #print("raiising error b/c repeat")
            raise SchemeError
        formal.append(formals.first)
        formals = formals.second
        #print("new formal list", formal)
        #print("new formals:", formals)
